...
def main(page: Page):
    ...
    def draw(e):
        ...
        try:
            values = list(
                map(lambda t: solver.solve(funcs.replace("@x", valSet(str(t)))), data)
            )
            ...
            column = Column(
                [rowf, rowrange, PlotlyChart(fig, expand=False)],
            ...
            column = Column(
                [rowf, rowrange, Text(str(e), size=40, color=ft.colors.RED)],
                scroll=ft.ScrollMode.ALWAYS,
            )
            page.clean()
            page.add(column)
